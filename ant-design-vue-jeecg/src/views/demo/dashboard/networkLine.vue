...
<template>
  <div id="networkLineChart" :style="{width: '90%', height: '400px'}"></div>
</template>
...
export default {
    ...
    data () {
        return {
            ...
            networkMetrics: 'metrics/cpu/In._avg[1589781782,1589785382,15],metrics/cpu/Out._avg[1589781782,1589785382,15],metrics/cpu/User._avg[1589781782,1589785382,15],metrics/cpu/Idle._avg[1589781782,1589785382,15]',
    ...
    methods: {
        drawLine () {
            let mychart = echarts.init(document.getElementById('networkLineChart'))
        ...
        networkclust () { // 初始化CPU集群整体情况
            this.nowStamp = Date.parse(new Date()) - 180000
            var networkMetrics = this.networkMetrics
            var dateString = '[' + (this.nowStamp / 1000 - 3600) + ',' + (this.nowStamp / 1000) + ']'
            console.log(dateString)
            networkMetrics = 'metrics/network/In._avg' + dateString + ',' + 'metrics/network/Out._avg' + dateString
            axios.get('/ambari/api/v1/clusters/HT706',
                ...
                var cacheAVGdata = response.data.metrics.network['In._avg']
                var totalAVGdata = response.data.metrics.network['Out._avg']
